...
if [[ ! -f "$FILE" ]]; then
	...
	polybar-msg cmd show && notify-send -a bar -i custom-switch-off Polybar "Polybar is now visible."
else
	...
	polybar-msg cmd hide && bspc config bottom_padding 0 && notify-send -a bar -i custom-switch-off Polybar "Polybar is now hidden. Unhide with Super + Shift + Slash."
	rm "$FILE"
fi
