...
rofi_command="rofi -p Screenshot -dmenu -selected-row 0 -theme $XDG_CONFIG_HOME/rofi/shot.rasi"
...
function notify_view() {
  notify-send -i custom-list -a shot_icon -u low Screenshot "Copied to clipboard."
  xdg-open "$dir/$file"
  if [[ -e "$dir/$file" ]]; then
